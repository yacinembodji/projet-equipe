    ...
    <header className="bg-white shadow-sm">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 mx-auto">
          ...
          <NavLink to="/service" className={({ isActive }) => `${linkClass} ${isActive ? activeClass : "text-gray-700"}`}>
            Services
          </NavLink>
        </nav>
      ...
      {open && (
        <nav className="md:hidden px-6 pb-4 space-y-2">
          ...
          <NavLink to="/service" onClick={() => setOpen(false)} className={({ isActive }) => `block ${linkClass} ${isActive ? activeClass : "text-gray-700"}`}>
            Services
          </NavLink>
        </nav>
      )}
